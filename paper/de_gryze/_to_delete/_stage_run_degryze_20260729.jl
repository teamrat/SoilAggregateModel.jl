...
# --- Panel 1: MWD — the paper's own comparison quantity ---
# Model: eq. (1) fixed-weight MWD over the four sieve classes, so it is
# constructed the same way as the measured values. Data: degryze2006.csv,
# 9 sampling times x 5 soils, mm. Measurements are markers; only the model is
# drawn as a line.
...
if df_mwd_data !== nothing
    ...
    for (i, col) in enumerate(names(df_mwd_data)[2:end])
        ...
        mask = .!ismissing.(vals)
        any(mask) || continue
        # The simulated soil is filled and larger; the others are open, for
        # context. All are markers — none of this is a fitted curve.
        is_this = (i == SOIL_ID)
        scatter!(p1, t_mwd[mask], Float64.(vals[mask]),
                 color = soil_colors[i], label = soil_names[i],
                 ms = is_this ? 7 : 4,
                 markershape = is_this ? :circle : :diamond,
                 markerstrokewidth = is_this ? 1.2 : 0.4,
                 markeralpha = is_this ? 1.0 : 0.55)
    end
end

# --- Panel 2: large macroaggregates ---
# The Table 3 lines are NOT measurements. They are De Gryze's fitted formation
# rates (% > 2 mm per day, R^2 0.85-0.94) rendered as slope*t. Styled thin and
# dashed, and labelled as rates, so they cannot be read as data series.
...
for a in DEGRYZE_OBSERVED
    plot!(p2, 0:21, (0:21) .* a.slope_gt2000,
          ...
          color = soil_colors[a.id],
          alpha = a.id == SOIL_ID ? 1.0 : 0.5,
          label = "S$(a.id) rate $(a.slope_gt2000) %/d (R²=$(a.R2))")
end
...
p3 = plot(df_pop.time_days, co2_model_ugC_per_gsoil,
          lw=2.5, color=:black, label="Model (soil $(SOIL_ID))",
...
if df_co2_data !== nothing
    ...
    for (i, col) in enumerate(names(df_co2_data)[2:end])
        ...
        any(mask) || continue
        is_this = (i == SOIL_ID)
        scatter!(p3, t_co2[mask], Float64.(vals[mask]),
                 color = soil_colors[i], label = soil_names[i],
                 ms = is_this ? 7 : 4,
                 markershape = is_this ? :circle : :diamond,
                 markerstrokewidth = is_this ? 1.2 : 0.4,
...
# --- Panel 4: CO₂ flux. No measured counterpart — the paper reports cumulative
# respiration only, so this panel is model diagnostic, not a comparison. ---
...
plot!(p5, df_pop.time_days, df_pop.pct_lt53um,     lw=2, label="< 0.05 mm", ls=:dash)
