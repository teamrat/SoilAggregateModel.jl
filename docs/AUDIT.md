...
**~~Van Genuchten — four implementations~~ — CLOSED, and already closed when I re-reported it as open on 07-30.** Both inline copies were replaced during the `s_M` work: `initial_conditions.jl:361` and `api.jl:184` call `van_genuchten`, and `api.jl` carries a comment naming the `DomainError` at saturation. The other two "copies" were `run_simulations.jl` (now archived) and `_to_delete/profiles.jl`. **This entry sat stale for a day and I restated it from the audit text instead of re-checking the tree — the same failure as A3.** Verify against the tree before reporting.

Left in place deliberately: `m = 1.0 - 1.0/n` is written in both `van_genuchten` and `van_genuchten_inverse`, thirty lines apart in one file. Two sites, one arithmetic operation, and a reader checks each formula against the paper — extracting it would put indirection inside the thing being verified. Flagged, not fixed.
...
**~~Arrhenius — three~~ — CLOSED 07-30, and the earlier "CLOSED" was premature.** Fixing `D_fungal_translocation` closed one of the three and I logged the item as done. The other two: `arrhenius_ratio` computed `exp(Ea/R·(1/T1 − 1/T2))`, which IS `arrhenius(Ea, T2, T1)`; and `henry_vant_hoff` computed `exp(ΔH/R·(1/T − 1/T_ref))`, which IS `arrhenius_ratio(ΔH, T, T_ref)` — van't Hoff and Arrhenius are one exponential with ΔH in place of Ea. Both delegate now, both bitwise because the expressions are identical term for term, and `arrhenius` is the only exponential of this form left in the package. `test_temperature.jl` asserts both identities with `==`, so re-expanding either fails the suite.

**~~Langmuir–Freundlich — three~~ — CLOSED, also already closed when re-reported.** All four call sites — `api.jl:190`, `reactions.jl:164`, `derived.jl:113`, `initial_conditions.jl:187` — call `M_eq_langmuir_freundlich` in `maoc.jl:72`. One implementation.

**~~Sigmoid threshold — four implementations~~ — CLOSED 07-30. Was not in this audit.** `1/(1 + exp(-β(x − x_min)))` with `β = 50/x_min` was written out in `h_B`, `h_E` and `h_Fi`, and a fourth time as the POM activation delay in **both** `timestepper.jl` and `mol.jl` with `β = 1/(0.1·t_delay)`. That the delay is the same switch was invisible from either write-out. `sigmoid_threshold(x, x_min, steepness)` in `math_utils.jl` is the single definition; `steepness = β·x_min`, so it is scale-free in `x` and serves both a pool in µg/mm³ and a delay in days. The two bare `50.0`s and the `0.1` became `SIGMOID_STEEPNESS = 50` and `POM_DELAY_STEEPNESS = 10`; neither has a citation. The three `h_*` are bitwise. `pom_delay_factor(t, t_delay)` returns exactly 1.0 for `t_delay ≤ 0` — every configuration in `paper/` — so **every current run is bitwise**; at `t_delay > 0` it differs sub-ulp because `β·(t − t_delay)` replaces `(t − t_delay)/(0.1·t_delay)`. The delay had **no test in either solver**; it has one now, including the documented 90 %-by-`t ≈ 12.2` width.

**Left as a judgment call: `exp(ν·ψ)` at two sites** — `bacteria.jl:77` and `fungi.jl:83`, the water-stress factor on respiration, identical in form with different `ν`. A `water_stress(ν, ψ)` primitive would be trivially bitwise, but there is no shared biology file to hold it and inventing one is the proliferation §E objects to. Not decided unilaterally.

**~~Sorption retardation `θ + ρ_b·k_d` — eight sites with no owning function~~ — CLOSED.** `sorption_capacity(θ, ρ_b, k_d)` and `C_aqueous(C, θ, soil)` in `maoc.jl` own it; all six `src/` sites route through them. `test_biology.jl` asserts the defining identity and that the relation is linear in `C`, so the ω dilution passes through it.
