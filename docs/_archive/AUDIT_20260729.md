...
| 07-30 | **A4 decided: keep both solvers** | **Instrument rewritten, not un-forked.** The reason to keep the split solver stands on its own — independent discretisation, and it closes the carbon balance as a measurement where the stiff path closes it by construction. `paper/de_gryze/verify_solver_agreement.jl` replaces the archived `converge_doc.jl`: all eight fields plus `P`, `CO₂`, `r_agg` at day 22, the split solver's own balance error, and a `dt_min` refinement to say whether a disagreement is under-resolution or something else. Written fresh because the old script's premise was measured at `k_L = 1000` against today's 25000. **Not yet run — REFERENCE §17a/§20a stays unsupported until it is.** C is next: it makes this an invariant rather than a script someone remembers. |
| 07-30 | **A5 + A6 disposition, and the prune** | **ARCHIVED, not repaired.** Criterion: does it contribute to the forward run. `optimize_soil3.jl`, `fitting/` (A6's subject), `diagnostics/` and `legacy/` — 2070 lines — moved to `paper/_archive/degryze_tooling_20260730/` with a README saying what each was and why none of it is quotable. `paper/de_gryze/` is down to nine entries and `degryze_config.jl` has one consumer. Dangling references repaired in `src/constants.jl`, `run_degryze.jl`, `degryze_config.jl`, `BACKLOG.md` and the folder README. **Side effect that matters:** archiving `converge_doc.jl` leaves the two-solver agreement claim with no live instrument, which forces A4's hand — either the split solver goes and the claim goes with it (making C the prerequisite), or the agreement script is rewritten. Calibration is now explicitly a future project: hand-tune to close, then one reusable fitting routine for all examples, tested on De Gryze. |
...
### A4. ~~`converge_doc.jl` is the cited evidence for solver agreement, and it runs different parameters~~ — **DECIDED 2026-07-30: keep both solvers, instrument rewritten. Awaiting one run.**
...
Production runs `1e-4`, `1.5`, `0.005`, `0.05`, `1.0`. **The agreement was demonstrated for a model that is no longer the model.** All four scripts in `diagnostics/` had the same problem; `diagnose_speed.jl:42-43` stated the requirement explicitly — "These MUST match run_degryze.jl exactly" — and no longer met it.

**Decision (owner, 07-30): keep the split solver.** The earlier plan — archive it and delete the agreement claim — is off. The reason to keep it stands on its own: it is an independent discretisation of the same equations, and it tracks respired carbon separately, so it closes the carbon balance as a *measurement* where the stiff path recovers CO₂ by difference and closes it by construction. Nothing else in the repo performs that check.

**`paper/de_gryze/verify_solver_agreement.jl`** replaces `converge_doc.jl`. Written fresh rather than un-forked, because the archived script's stated premise ("9.6 % on C at day 45") was measured at `k_L = 1000` against the current 25000 — the one parameter that moves DOC — so its narrative was about numbers that no longer exist. ~90 lines of code on `degryze_config.jl`. It is the only file in `paper/de_gryze/` that is not part of the forward run; it is there because it needs a configuration and the package must not depend on one.

Two parts. Part 1 compares all eight fields plus `P`, `CO₂` and `r_agg` at day 22, and reports the split solver's own carbon-balance error — the thing it is kept for. Part 2 is what settles a disagreement: refine the split solver's `dt_min` from 1e-4 to 1e-5 and see whether its answer moves toward the stiff one. Its controller halves Δt only until `max|S·Δt|/u ≤ 0.10`, so a lower floor does not force small steps everywhere; it only lets Δt fall where the criterion asks. Movement means the production split run was under-resolved. No movement means the disagreement is not a resolution problem and neither answer should be quoted until it is explained.

The expected tolerance is stated in the script rather than assumed: `mol.jl` documents three deliberate differences — the split solver lags θ and the diffusivities by one step, clips negatives into CO₂, and accumulates CO₂ as a running sum. 1e-3 is the lag. 1e-1 is a bug.

**Status: written, not yet run.** Until it is, REFERENCE §17a/§20a remains unsupported. The claim there is only as current as the last run of this script, which the script now says in its own closing output.

### A5. ~~`optimize_soil3.jl` is a complete forked configuration~~ — **CLOSED 2026-07-30: archived, not repaired**
...
**Disposition (owner, 07-30).** Not repaired. Calibration is a separate future project with a different shape: get the fit visually close by hand, then build **one** reusable fitting routine central enough for every example to use, and test it against De Gryze. Repairing a soil-3-specific 663-line fork is not a step toward that.

So the criterion applied was *does this contribute to the forward run we have* — and everything outside `src/` that did not was archived to `paper/_archive/degryze_tooling_20260730/` with a README stating what each file was for and why none of it should be quoted:

| archived | lines | why |
|---|---|---|
| `optimize_soil3.jl` | 663 | forked config; also hard-coded the measured soil-3 CO₂ series that `data/degryze_CO2_2006.csv` holds — the data duplicated into a script |
| `fitting/` (5 files) | 667 | architecturally sound and all on the shared config, but calibration is the future project. **This is where A6 lands too**: `sensitivity.jl` was A6's subject, and the same delete-rather-than-patch reasoning applies. `setup.jl` is the piece to read when the routine gets built |
| `diagnostics/` (4 files) | 740 | `smoke_stiff.jl` and `bench_stiff.jl` answered questions that are permanently answered; `converge_doc.jl`'s premise was measured at `k_L = 1000` against today's 25000; `diagnose_speed.jl` is current, and its numbers are quoted in §B so the result survives |
| `legacy/` (2 R scripts) | — | superseded plotting; the R layer gets built once Julia is settled |

`paper/de_gryze/` is now nine entries: `run_degryze.jl`, `degryze_config.jl`, `degryze_soils.jl`, `postprocess_dataframe.jl`, `data/`, `output/`, `README.md` and the two specs.

Dangling references repaired in the same pass: `src/constants.jl`, `run_degryze.jl`, `degryze_config.jl` (three sites), `docs/BACKLOG.md` (two), and `paper/de_gryze/README.md` (layout, the one-configuration claim, the solver note, and the Fitting section rewritten as a statement of the future project rather than a description of files). The frozen audits (`STRUCTURAL_AUDIT_2026-07-27.md`, `TEST_AUDIT_2026-07-28.md`) still name these files and are left alone — they are historical records.

**Consequence for A4, and it is the one thing this pass makes worse before it makes it better.** `converge_doc.jl` was the only script asking whether the two solvers agree on DOC, and it is now archived. The two-solver agreement claim in `timestepper.jl` and REFERENCE §17a/§20a is therefore unsupported *and* has no live instrument. Two ways out, and A4 has to pick one: archive the split solver (A4 as planned) and delete the agreement claim with it, since with one solver there is nothing to agree — which makes **C the real prerequisite**, because the stiff solver then has no independent check and currently no test; or keep the split solver and write the agreement script fresh against the current config. **Decided the same day: keep it, script written — see A4.**

### A6. ~~A stale runtime warning is blocking the calibration path~~ — **CLOSED 07-29** (deleted); its file archived 07-30
...
## B. ~~Will produce wrong numbers soon — duplication~~ — **CLOSED 2026-07-30**

Every item below is closed. Four passes: `M_max` and the `k_ma` units; the DOC partition denominator and the gas-phase literals; the conservation weight and the system-carbon total; the sigmoid threshold and the Arrhenius family. The cost arm closed on measurement rather than on work. Two entries turned out to have been closed already and are marked as retracted where I re-reported them.

The section is kept rather than deleted because the *pattern* is the finding, not the individual sites: in every case the fix had been applied to the instance in view and not to the class, and in two cases I restated an entry from this file instead of re-checking the tree.

**~~Configuration~~ — CLOSED 07-30.** Consolidation had been applied to the two files in view at the time; five of eleven consumers still held their own copy. `diagnose_speed.jl` was un-forked, then it and the other four (`optimize_soil3.jl`, `bench_stiff.jl`, `converge_doc.jl`, `smoke_stiff.jl`) together with all of `fitting/` and `legacy/` were archived to `paper/_archive/degryze_tooling_20260730/` — everything outside `src/` that did not contribute to the forward run. `degryze_config.jl` now has exactly one consumer, `run_degryze.jl`. `degryze_config.jl:7-9` had recorded the previous instance of exactly this failure; the fix had been applied to the instance, not the class, which is what left five.
...
2. ~~**A2**~~ — done 07-29.
3. ~~**Deferred to the rebuild** — A1, A4, A5.~~ A1 archived 07-29, A5 archived 07-30. Nothing under `paper/_archive/` may be quoted. **A4 is now the only one open, and archiving `converge_doc.jl` sharpened it**: the two-solver agreement claim in `timestepper.jl` has no live instrument, so either the split solver goes and the claim goes with it — making **C the prerequisite**, since the stiff solver then has no independent check and no test — or the split solver stays and the agreement script is written fresh. See A5.
4. **One test calling `run_aggregate_stiff` end-to-end**, plus one pinning `mol_laplacian` against `crank_nicolson_step!`. The default solver and the sanctioned duplication are both currently unguarded. Now the next item after A4's script has been run once: with both solvers kept, C is what turns the agreement check from a script someone remembers to run into an invariant.
...
## ω — CLOSED 2026-07-30. This section was the problem.

**The ω convention was settled in `SI_tessellation.tex` and the manuscript before
this section was ever written.** S4 defines `f_p`, `f_m`, `ω = (f_m/f_p)³` and
accepts overlap as the design; S5 gives the dilution with the residue mass exempt;
S6 proves the population total exact for diffusion and for reactions linear in the
diluted variables and derives the Monod error; S7 bounds it; **S8 is the normative
implementation table and the code matches it row for row.** The manuscript's fifth
`r_agg` property closes the companion question about the two binder scalings, with
the residual carried by the fitted `κ_b`.

What stood here was a list of "four open questions" from an independent trace on
07-29, together with a plan to remove the dilution from `create_initial_state` and
insert `1/ω` at every summation site. **That plan was wrong and would have
double-corrected**: dilution × volume-inflation already cancel, which is what S6
proves and what the later map of the code confirmed — ω appears in exactly one
place and in no summation. Of the four questions, three were not objections to the
convention (oxygen is boundary-supplied and correctly undiluted; S7's argument
needs the *soluble* background, which the partition puts at `C = 0.344` against
`M = 28.66` µg/mm³ for soil 3; the remaining nonlinear terms fall under the same
`1/ω` bound). The fourth was a dangling cross-reference in the main text, fixed
07-30. REFERENCE §26 erratum 13 has the full disposition.

**Why this is recorded rather than deleted.** The convention was closed in the
manuscript and SI, this file was not updated when the manuscript was, and then
this file was read back as though it were evidence — for a full day, including by
me, twice in one exchange. Together with the Van Genuchten and Langmuir–Freundlich
entries in §B, that is three stale internal notes reported as live findings in one
session. The failure is not the staleness; it is treating a note as a source. When
this file and the code or the manuscript disagree, **the note is the thing that is
wrong** until checked.

*Still genuinely open, and independent of ω:* `κ_s_ref` (0.1 → 0.01, undocumented
at the time) and `κ_d_ref` (0.01 → 1e-4). The ratio went 10 → 100 — tenfold
stronger hysteresis — changed silently, and the elasticity that justified
`κ_d_ref` was measured while the `M_max` bug was live. These were parked behind ω;
ω is not the blocker, so they are simply open. Re-examine on the next run.
...
- ~~**`optimize_soil3.jl`** — repair or delete?~~ Settled 07-30: archived, along with everything else outside `src/` that did not contribute to the forward run. Calibration is a separate future project.
